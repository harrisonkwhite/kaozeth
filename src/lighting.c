...
typedef struct {
    zfw_s_vec_2d_int* buf;
    int cap;
    int start;
    int len;
} s_light_pos_queue;

static bool IsLightPosQueueValid(const s_light_pos_queue* const queue, const zfw_s_vec_2d_int map_size) {
    if (!queue->buf
        || queue->cap <= 0
    ...
    for (int i = 0; i < queue->len; i++) {
        ...
        const zfw_s_vec_2d_int pos = queue->buf[bi];
...
static bool EnqueueLightPos(s_light_pos_queue* const queue, const zfw_s_vec_2d_int light_pos, const zfw_s_vec_2d_int map_size) {
...
static zfw_s_vec_2d_int DequeueLightPos(s_light_pos_queue* const queue, const zfw_s_vec_2d_int map_size) {
#if DO_LIGHT_POS_QUEUE_VALIDITY_CHECKS
    assert(IsLightPosQueueValid(queue, map_size));
#endif
    assert(queue->len > 0);

    const zfw_s_vec_2d_int light = queue->buf[queue->start];
...
bool PropagateLights(const s_lightmap* const lightmap, s_mem_arena* const temp_mem_arena) {
    ...
    s_light_pos_queue light_pos_queue = {
        .buf = MEM_ARENA_PUSH_TYPE_CNT(temp_mem_arena, zfw_s_vec_2d_int, light_limit),
        .cap = light_limit
    };
    ...
    for (int y = 0; y < lightmap->size.y; y++) {
        for (int x = 0; x < lightmap->size.x; x++) {
            const zfw_s_vec_2d_int pos = {x, y};
    ...
    while (light_pos_queue.len > 0) {
        const zfw_s_vec_2d_int pos = DequeueLightPos(&light_pos_queue, lightmap->size);

        const zfw_s_vec_2d_int neighbour_pos_offsets[] = {
            {0, 1},
            {0, -1},
            {1, 0},
            {-1, 0},
        ...
        for (int i = 0; i < STATIC_ARRAY_LEN(neighbour_pos_offsets); i++) {
            const zfw_s_vec_2d_int neighbour_pos = ZFW_Vec2DS32Sum(pos, neighbour_pos_offsets[i]);
...
void RenderLightmap(const zfw_s_rendering_context* const rendering_context, const s_lightmap* const map, const zfw_s_vec_2d pos, const float tile_size) {
    ...
    for (int ty = 0; ty < map->size.y; ty++) {
        for (int tx = 0; tx < map->size.x; tx++) {
            const t_light_level light_lvl = LightLevel(map, (zfw_s_vec_2d_int){tx, ty});
