...
void AddTile(s_tilemap_core* const tm_core, const zfw_s_vec_2d_i pos, const e_tile_type tile_type) {
    ...
    ZFWActivateBit(ZFWIndexFrom2D(pos, TILEMAP_WIDTH), (zfw_t_byte*)tm_core->activity, TILEMAP_WIDTH * TILEMAP_HEIGHT);
...
void RemoveTile(s_tilemap_core* const tm_core, const zfw_s_vec_2d_i pos) {
    ...
    ZFWDeactivateBit(ZFWIndexFrom2D(pos, TILEMAP_WIDTH), (zfw_t_byte*)tm_core->activity, TILEMAP_WIDTH * TILEMAP_HEIGHT);
}

zfw_s_rect_edges_i RectTilemapSpan(const zfw_s_rect rect) {
    assert(rect.width >= 0.0f && rect.height >= 0.0f);

    return ZFWRectEdgesIClamped(
        (zfw_s_rect_edges_i){
            rect.x / TILE_SIZE,
            rect.y / TILE_SIZE,
        ...
        (zfw_s_rect_edges_i){0, 0, TILEMAP_WIDTH, TILEMAP_HEIGHT}
    );
}
...
bool TileCollisionCheck(const t_tilemap_activity* const tm_activity, const zfw_s_rect collider) {
    ...
    const zfw_s_rect_edges_i collider_tilemap_span = RectTilemapSpan(collider);
    ...
    for (int ty = collider_tilemap_span.top; ty < collider_tilemap_span.bottom; ty++) {
        for (int tx = collider_tilemap_span.left; tx < collider_tilemap_span.right; tx++) {
            ...
            if (ZFWDoRectsInters(collider, tile_collider)) {
                return true;
            }
        }
    }
...
void ProcTileCollisions(zfw_s_vec_2d* const pos, zfw_s_vec_2d* const vel, const zfw_s_vec_2d collider_size, const zfw_s_vec_2d collider_origin, const t_tilemap_activity* const tm_activity) {
    ...
    const zfw_s_rect diag_collider = Collider(ZFWVec2DSum(*pos, *vel), collider_size, collider_origin);
...
void MakeContactWithTilemapByJumpSize(zfw_s_vec_2d* const pos, const float jump_size, const zfw_e_cardinal_dir dir, const zfw_s_vec_2d collider_size, const zfw_s_vec_2d collider_origin, const t_tilemap_activity* const tm_activity) {
    ...
    const zfw_s_vec_2d jump = ZFWVec2DScaled(zfw_g_cardinal_dir_vecs[dir], jump_size);

    while (!TileCollisionCheck(tm_activity, Collider(ZFWVec2DSum(*pos, jump), collider_size, collider_origin))) {
        *pos = ZFWVec2DSum(*pos, jump);
    }
}
...
void RenderTilemap(const zfw_s_rendering_context* const rendering_context, const s_tilemap_core* const tilemap_core, const t_tilemap_tile_lifes* const tilemap_tile_lifes, const zfw_s_rect_edges_i range, const zfw_s_textures* const textures) {
    ...
    for (int ty = range.top; ty < range.bottom; ty++) {
        for (int tx = range.left; tx < range.right; tx++) {
            if (!IsTileActive(&tilemap_core->activity, (zfw_s_vec_2d_i){tx, ty})) {
                continue;
            }
