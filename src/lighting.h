...
static inline bool IsLightPosInBounds(const s_v2_int pos, const s_v2_int map_size) {
    return pos.x >= 0 && pos.x < map_size.x && pos.y >= 0 && pos.y < map_size.y;
}

static inline t_light_level LightLevel(const s_lightmap* const map, const s_v2_int pos) {
...
static inline void SetLightLevel(const s_lightmap* const map, const s_v2_int pos, const t_light_level lvl) {
    assert(IsLightPosInBounds(pos, map->size));
